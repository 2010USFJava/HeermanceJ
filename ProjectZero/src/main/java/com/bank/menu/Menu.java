...
public class Menu {
	...
	public static void customerMenu(Customer cust) {//need to add in option to choose account
...
//			System.out.println("Please select which account you would like to access:");
	...
	public static void employeeMenu(String username) {//still need to transfer
	...
	public static void approveOrDeny(String username) {
		...
		if(review==0) {
			...
			Customer a = AllFiles.cList.get(review-1);
			a.toString();
			System.out.println("Would you like to approve this account? (y/n)");
			String approve = scan.nextLine();
			if(approve.equalsIgnoreCase("y")) {
				AllFiles.accList.get(review-1).setAccountStatus(true);
				a.setAccount(a.getAccount()+1);
				System.out.println("You have approved this account.");
				LogThis.LogIt("info", "A " + AllFiles.accList.get(review-1).getType() + " account for " + a.getUsername() + " has been approved by " + username + ".");
				approveOrDeny(username);
			}else if(approve.equalsIgnoreCase("n")) {
				System.out.println("You have denied this account.");
				AllFiles.accList.remove(review-1);
				LogThis.LogIt("info", "A " + AllFiles.accList.get(review-1).getType() + " account for " + a.getUsername() + " has been denied by " + username + ".");
				approveOrDeny(username);
			}
		}
